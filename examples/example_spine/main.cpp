...
#include "../../framework_spine/framework_spine.h"
...
struct app_t {
    ...
    spine_prototype_t proto_;
    spine_instance_t inst_;
    ...
    bool app_init() {
        ...
        if (!proto_.load("assets/spineboy.atlas", "assets/spineboy-ess.json")) {
          return false;
        }
        if (!inst_.load(proto_)) {
          return false;
        }
        inst_.set_animation(0, "run", true);
    ...
    bool tick() {
        ...
        inst_.tick(0.1f / _FPS);
        inst_.draw(service_.draw_);
