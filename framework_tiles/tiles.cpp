...
bool collision_map_t::collide(const rectf_t& r, vec2f_t& out)
{
    ...
    const float ival = maxv(r.x1 - r.x0, r.y1 - r.y0);
    out.x = ival;
    out.y = ival;
    ...
    vec2f_t& out)
{
    ...
    // set the worst case resolution to improve upon
    const float ival = maxv<float>(r.x1 - r.x0, r.y1 - r.y0);
    out.x = ival;
    out.y = ival;
    ...
    for (int32_t y = miny; y <= maxy; ++y) {
        for (int32_t x = minx; x <= maxx; ++x) {
            ...
            for (size_t i = 0; i < vec.size(); ++i) {
                if (!set || vec2f_t::length(vel + vec[i]) < vec2f_t::length(vel + best)) {
                    best = vec[i];
                    set = true;
                }
            }
...
bool collision_map_t::collide_alt(const rectf_t& r, vec2f_t& out)
{
    ...
    // set the worst case resolution to improve upon
    const float ival = maxv(r.x1 - r.x0, r.y1 - r.y0) + 1;
    out.x = out.y = 0.f;
    float best = ival;
    ...
    for (int32_t y = miny; y <= maxy; ++y) {
        for (int32_t x = minx; x <= maxx; ++x) {
            ...
            const uint32_t all = e_tile_push_up | e_tile_push_down | e_tile_push_left | e_tile_push_right;
            if ((t & all) == all)
                continue;
            ...
            if (mx | my) {
                if ((split = minv(mx ? absv(sx) : ival, my ? absv(sy) : ival)) < best) {
                    out.x = sx;
                    out.y = sy;
                    best = split;
                }
...
bool collision_map_t::line_of_sight(const vec2f_t& a, const vec2f_t& b)
{
    ...
    if (a.y < b.y) {
        if (a.x < b.x) {
            return !((hit.y >= a.y) && (hit.y <= b.y) && (hit.x >= a.x) && (hit.x <= b.x));
        } else {
            return !((hit.y >= a.y) && (hit.y <= b.y) && (hit.x >= b.x) && (hit.x <= a.x));
        }
    } else {
        if (a.x < b.x) {
            return !((hit.y >= b.y) && (hit.y <= a.y) && (hit.x >= a.x) && (hit.x <= b.x));
        } else {
            return !((hit.y >= b.y) && (hit.y <= a.y) && (hit.x >= b.x) && (hit.x <= a.x));
        }
    }
}
} // namespace tengu
